...
namespace channel {
  struct ChannelShutDownException : public std::runtime_error {
    public:
      ChannelShutDownException() : std::runtime_error("Channel has been shut down") {}

      const char * what () const throw () {
         return "Channel has been shut down";
      }
  };
  ...
  template<typename T>
  std::pair<Sender<T>, Receiver<T>> mkChannel() {
